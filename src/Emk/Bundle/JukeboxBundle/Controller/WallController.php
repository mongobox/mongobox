...
use Emk\Bundle\JukeboxBundle\Entity\Vote;
use Emk\Bundle\JukeboxBundle\Entity\VideoCurrent;
use Emk\Bundle\JukeboxBundle\Form\VideoType;
...
class WallController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $total_video = count($em->getRepository('EmkJukeboxBundle:Videos')->findAll());
        $playlist = $em->getRepository('EmkJukeboxBundle:Playlist')->next(10);
        $videos_historique = $em->createQuery('SELECT v FROM EmkJukeboxBundle:Videos v ORDER BY v.lastBroadcast DESC')->setMaxResults(5)->getResult();
        $video_en_cours = $em->getRepository('EmkJukeboxBundle:VideoCurrent')->findAll();
        $somme = $em->getRepository('EmkJukeboxBundle:Vote')->sommeVotes($video_en_cours[0]->getId());
        $somme_pl = $em->getRepository('EmkJukeboxBundle:Vote')->sommeAllVotes();
        ...
        if ( 'POST' === $request->getMethod() ) {
            ...
            if ( $form->isValid() ) {
                ...
                $isVideo = $em->getRepository('EmkJukeboxBundle:Videos')->findOneby(array('lien' => $video->getLien()));
    ...
    public function jukeboxAction(Request $request)
    {
        ...
        if (count($video_en_cours) > 0) {
            $total_vote = $em->getRepository('EmkJukeboxBundle:Vote')->sommeVotes($video_en_cours[0]->getId());
            $video_en_cours[0]->getId()->setVotes($video_en_cours[0]->getId()->getVotes() + $total_vote);
            //On wipe les votes de la vidéo d'avant !
            $em->getRepository('EmkJukeboxBundle:Vote')->wipe($video_en_cours[0]->getId()->getId());
        }

        //On regénère la playlist
        $em->getRepository('EmkJukeboxBundle:Playlist')->generate();

        //On va chercher la prochaine vidéo de la playlist
        $playlist_next = $em->getRepository('EmkJukeboxBundle:Playlist')->next(1);
        ...
        //$video_old = $em->getRepository('EmkJukeboxBundle:Videos')->next();
        //$video = $em->getRepository('EmkJukeboxBundle:Videos')->find($video_old->getId());

        $em->getRepository('EmkJukeboxBundle:VideoCurrent')->wipe();
    ...
    {
        $em = $this->getDoctrine()->getEntityManager();
        $video_current = $em->getRepository('EmkJukeboxBundle:VideoCurrent')->findOneBy(array('id_video' => $id_video));
        $video = $em->getRepository('EmkJukeboxBundle:Videos')->find($id_video);

        //On wipe les votes pour cette vidéo
        $em->getRepository('EmkJukeboxBundle:Vote')->wipe($video->getId());
    ...
    public function voteAction(Request $request, $video, $sens, $current = false)
    {
        ...
        $old_vote = $em->getRepository('EmkJukeboxBundle:Vote')
						->findOneBy(array(
							'user'	=> $userDb,
							'video' => $video, 
        ...
        return new Response($em->getRepository('EmkJukeboxBundle:Vote')->sommeVotes($video));
    }
    ...
    /**
     * @Template("EmkJukeboxBundle:Wall/Blocs:videoEnCours.html.twig")
    ...
    /**
     * @Template("EmkJukeboxBundle:Wall/Blocs:statistiques.html.twig")
    ...
    /**
     * @Template("EmkJukeboxBundle:Wall/Blocs:fluxRss.html.twig")
