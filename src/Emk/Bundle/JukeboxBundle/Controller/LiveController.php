...
class LiveController extends Controller
{
	...
	protected function _initJukebox()
	{
		...
		$results = $em->getRepository('EmkJukeboxBundle:VideoCurrent')->findAll();
		if (count($results) > 0) {
			$currentPlayed = $results[0];

			$currentVideo	= $em->getRepository('EmkJukeboxBundle:Videos')->findOneby(array(
				'id' => $currentPlayed->getId()
			));

			$votes = $em->getRepository('EmkJukeboxBundle:Vote')->sommeVotes($currentPlayed);
			$currentVideo->setVotes($currentVideo->getVotes() + $votes);

			$em->getRepository('EmkJukeboxBundle:Vote')->wipe($currentPlayed->getId()->getId());
		}

		$em->getRepository('EmkJukeboxBundle:Playlist')->generate();

		$nextInPlaylist = $em->getRepository('EmkJukeboxBundle:Playlist')->next(1);
	...
	protected function _getVideoScores($videoId)
	{
		...
		$upVotes = count($em->getRepository('EmkJukeboxBundle:Vote')->findBy(array(
				'video'	=> $videoId,
				'sens'	=> self::UP_VOTE_VALUE,
		)));

		$downVotes	= count($em->getRepository('EmkJukeboxBundle:Vote')->findBy(array(
				'video'	=> $videoId,
    ...
    public function voteAction(Request $request)
    {
    	...
    	$currentVideo	= $em->getRepository('EmkJukeboxBundle:Videos')->findOneby(array('lien' => $videoId));
    	if (is_null($currentVideo) || !in_array($voteType, array('up', 'down'))) {
    		return new Response();
    	}

    	$oldVote = $em->getRepository('EmkJukeboxBundle:Vote')->findOneBy(array(
    	...
    	if ($currentVideo === 1) {
    		$currentPlayed = $em->getRepository('EmkJukeboxBundle:VideoCurrent')->findAll();
