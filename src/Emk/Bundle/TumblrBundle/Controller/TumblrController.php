<?php
namespace Emk\Bundle\TumblrBundle\Controller;
...
use Emk\Bundle\TumblrBundle\Form\TumblrType;
use Emk\Bundle\TumblrBundle\Entity\Tumblr;
use Emk\Bundle\TumblrBundle\Entity\TumblrVote;
...
class TumblrController extends Controller
{
    ...
    public function indexAction(Request $resquest, $page)
    {
        ...
        $tumblrRepository = $em->getRepository('EmkTumblrBundle:Tumblr');
    ...
    {
        $em = $this->getDoctrine()->getEntityManager();
        $tumblr_vote = $em->getRepository('EmkTumblrBundle:Tumblr')->find($id_tumblr);

        //Wipe de son ancien vote
        $old_vote = $em->getRepository('EmkTumblrBundle:TumblrVote')->findOneBy(array('tumblr' => $id_tumblr, 'ip' => $_SERVER['REMOTE_ADDR']));
        if (!is_null($old_vote)) {
            $em->remove($old_vote);
            $em->flush();
        }
    ...
    public function tumblrAction(Request $request)
    {
        ...
        $mongo_pute = $em->createQuery('SELECT t FROM EmkTumblrBundle:Tumblr t ORDER BY t.date DESC')->setMaxResults(5)->getResult();
        return array
        (
	...
	public function topAction(Request $resquest){
		...
		$em = $this->getDoctrine()->getEntityManager();
		$tumblrRepository = $em->getRepository('EmkTumblrBundle:TumblrVote');
