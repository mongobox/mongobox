...
class FavorisController extends Controller
{
    ...
    public function ajaxVideoAddToFavoriteAction($id_video)
    {
        ...
        $isAlreadyFavorite =
            $em->getRepository('MongoboxBookmarkBundle:UserFavoris')->checkUserFavorite($id_video, $user);
    ...
    public function voirFavorisAction($page)
    {
        ...
        $videos_user = $manager->getRepository('MongoboxBookmarkBundle:UserFavoris')->getUniqueFavorisUser(
            $user,
            $page,
            self::LIMIT_FAVORITES
        );

        // On définit s'il y a une page après celle courante
        $nextPage = false;
        if (count($videos_user) > self::LIMIT_FAVORITES) {
            unset($videos_user[self::LIMIT_FAVORITES]);
            $nextPage = true;
        }

        // On traite les vidéos pour avoir les infos directement dedans
        foreach ($videos_user as &$video) {
            $array_date_first_ajout =
                $manager->getRepository('MongoboxBookmarkBundle:UserFavoris')->getDateAddToFavorite(
                    $user,
                    $video['id']
                );
            $video = array_merge($video, $array_date_first_ajout);
            $video['listes'] = $manager->getRepository('MongoboxBookmarkBundle:ListeFavoris')->getListesUserForOneVideo(
                $user,
                $video['id']
            );
            foreach ($video['listes'] as &$liste) {
                $array_date_ajout = $manager->getRepository('MongoboxBookmarkBundle:UserFavoris')->getDateAddToList(
                    $user,
                    $video['id'],
                    $liste['id']
                );
        ...
        if ($request->isXmlHttpRequest()) {
            $html = $this->render(
                'MongoboxBookmarkBundle:Favoris/Listes:listeFavoris.html.twig',
                array('favoris' => $videos_user)
            )->getContent();
    ...
    public function removeVideoFromBookmarkAction($id_video)
    {
        ...
        $isRemoved =
            $em->getRepository("MongoboxBookmarkBundle:UserFavoris")->removeVideoFromBookmark($user, $id_video);

        $response = array(
            'success' => $isRemoved,
            'message' => ($isRemoved) ? "Vidéo supprimée avec succès de vos favoris" : "Echec lors de la suppression",
            'fav'     => $id_video
        );
    ...
    public function removeVideoFromListAction($id_video, $id_liste)
    {
        ...
        $fav = $em->getRepository("MongoboxBookmarkBundle:UserFavoris")->findOneBy(
            array(
                "user"  => $user,
    ...
    {
        $value = $this->get('request')->get('term');
        $em = $this->getDoctrine()->getManager();
        $user = $this->getUser();
